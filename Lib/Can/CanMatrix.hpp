...
public:
	...
	MatrixType getType() { return m_type; } const
...
protected:
	...
	unsigned long long setBit1(size_t length)
	{
	...
	unsigned long long setBit0(size_t length)
	{
		std::bitset<MATRIX_LENGTH> bit;
		for (size_t i = 0; i < length; i++)
			bit.set(i, false);
		...
		if (m_type == MatrixType::MT_INTEL)
		{
			...
			if (offset > 8)
			{
				...
				for (int i = startPos; i <= endPos; i++)
				{
					...
					{
						int tempOffset = (i - startPos - 1) * 8 + firstDataLen;
		...
		else if (m_type == MatrixType::MT_MOTOROLA_LSB)
		{
			...
			if (offset > 8)
			{
				...
				for (int i = startPos; i <= endPos; i++)
				{
					...
					{
						int tempOffset = (endPos - i - 1) * 8 + lastDataLen;
						buffer[i] |= (data >> tempOffset) & 0xff;
					}
				}
			}
		...
		else if (m_type == MatrixType::MT_MOTOROLA_MSB)
		{
			...
			if (offset < 0)
			{
				...
				for (int i = startPos; i <= endPos; i++)
				{
					...
					{
						int tempOffset = (i - startPos - 1) * 8 + lastDataLen;
						buffer[i] |= (data >> tempOffset) & 0xff;
					}
				}
			}
