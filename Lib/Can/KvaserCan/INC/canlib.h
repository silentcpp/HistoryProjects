...
#define canHWTYPE_MINIHYDRA          62     // Eagle n嶪 Minihydra
#define canHWTYPE_EAGLE              62     // Eagle
...
kvStatus CANLIBAPI kvScriptStop(const CanHandle hnd, int slotNo); 

kvStatus CANLIBAPI kvScriptForceStop(const CanHandle hnd, int slotNo);

kvStatus CANLIBAPI kvScriptSendEvent(const CanHandle hnd, int slotNo, int eventType, int eventNo, unsigned int data);

kvEnvHandle CANLIBAPI kvScriptEnvvarOpen(const CanHandle hnd, char* envvarName, int *envvarType, int *envvarSize); // returns scriptHandle
...
kvStatus CANLIBAPI kvScriptEnvvarSetData(kvEnvHandle eHnd, void *buf, int start_index, int data_len);

kvStatus CANLIBAPI kvScriptEnvvarGetData(kvEnvHandle eHnd, void *buf, int start_index, int data_len);
...
kvStatus CANLIBAPI kvScriptLoadFile(const CanHandle hnd, int slotNo, char *filePathOnPC);
